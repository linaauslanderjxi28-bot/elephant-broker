...
4. **Embedding provider/model pairing** — The current Cognee configuration uses `EB_EMBEDDING_PROVIDER=openai_compatible` with the default `EB_EMBEDDING_MODEL=gemini/text-embedding-004` against the LiteLLM/OpenAI-compatible `/v1/embeddings` endpoint. If you change the model, run the probe-then-configure procedure (§ Probe-then-configure embedding dimensions above) and set `EB_EMBEDDING_DIMENSIONS` to the actual returned vector length. Do NOT guess the dimension from the model card — LiteLLM can truncate or alias.
