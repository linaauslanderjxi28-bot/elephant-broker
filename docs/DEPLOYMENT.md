...
1. **mistralai ghost package (legacy pip path only)** — older Cognee releases shipped a broken `mistralai` namespace package as a transitive dep that conflicts with `instructor`. With `uv` (the supported install path) and the current `cognee==0.5.6` pin, this is NOT an issue. The installer keeps a belt-and-suspenders cleanup step in case someone bypasses uv and runs `pip install` against the venv by habit.
...
4. **Embedding provider/model pairing** — The current Cognee 0.5.6 configuration uses `EB_EMBEDDING_PROVIDER=openai_compatible` with the default `EB_EMBEDDING_MODEL=gemini/text-embedding-004` against the LiteLLM/OpenAI-compatible `/v1/embeddings` endpoint. If you change the model, run the probe-then-configure procedure (§ Probe-then-configure embedding dimensions above) and set `EB_EMBEDDING_DIMENSIONS` to the actual returned vector length. Do NOT guess the dimension from the model card — LiteLLM can truncate or alias.
