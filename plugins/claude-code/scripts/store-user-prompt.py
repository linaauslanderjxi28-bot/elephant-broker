#!/usr/bin/env python3
"""Store the user's prompt until the Claude Code `Stop` hook provides the assistant answer.

Runs asynchronously on `UserPromptSubmit` so it does not block context lookup.
The prompt is staged and later paired with the assistant response to create a
single remembered QA-style turn.

Configuration:
    Resolves session state through the active runtime mode (`eb`, `http`, or
    `local_sdk`).
"""
