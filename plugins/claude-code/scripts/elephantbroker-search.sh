...
PLUGIN_DIR="${CLAUDE_PLUGIN_DATA:-${HOME}/.elephantbroker}"
...
import os
service_url = (os.environ.get("EB_SERVICE_URL") or os.environ.get("EB_RUNTIME_URL") or os.environ.get("COGNEE_SERVICE_URL") or os.environ.get("COGNEE_LOCAL_API_URL") or "http://localhost:8420").strip()
...
if (os.environ.get("EB_MODE") or "").strip().lower() in ("1", "true", "yes"):
    is_eb = True
elif os.environ.get("EB_SERVICE_URL") or os.environ.get("EB_RUNTIME_URL") or ":8420" in service_url:
    is_eb = True
if agent_name:
...
if [ "${EB_MODE:-}" = "true" ] || [ "${EB_MODE:-}" = "1" ] || [ "${EB_MODE:-}" = "yes" ]; then
    _IS_EB=true
elif [ -n "${EB_SERVICE_URL:-}" ] || [ -n "${EB_RUNTIME_URL:-}" ] || echo "${COGNEE_SERVICE_URL:-}" | grep -q ":8420"; then
    _IS_EB=true
fi
...
service_url = (os.environ.get("EB_SERVICE_URL") or os.environ.get("EB_RUNTIME_URL") or os.environ.get("COGNEE_SERVICE_URL") or os.environ.get("COGNEE_LOCAL_API_URL") or "http://localhost:8420").strip().rstrip("/")
api_key = (os.environ.get("COGNEE_API_KEY") or "").strip()
agent_name = (os.environ.get("COGNEE_AGENT_NAME") or "").strip()
...
if not api_key and service_url and agent_name and not is_eb:
    plugin_dir = pathlib.Path(os.environ.get("CLAUDE_PLUGIN_DATA") or pathlib.Path.home() / ".elephantbroker")
    cache_path = plugin_dir / "agent_keys.json"
    if cache_path.exists():
        try:
            cache = json.loads(cache_path.read_text())
...
try:
    if is_eb:
        ...
        def eb_search(req_query, req_scope=None, req_max_results=None, include_session_id=True, req_memory_class=None, req_entity_type=None):
            payload = {
                "query": req_query,
            ...
            if req_memory_class:
                payload["memory_class"] = req_memory_class
            if req_entity_type:
        ...
        if depth_mode == "deep":
            if mode == "graph":
                result = eb_search(query, "graph", req_max_results=top_k, req_memory_class="semantic", req_entity_type=entity_type)
            elif mode == "session":
            ...
            if mode == "session":
                ...
                result = eb_search(query, "graph", req_max_results=fast_top_k, include_session_id=False, req_memory_class="semantic", req_entity_type=entity_type)
            else:
