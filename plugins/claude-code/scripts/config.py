...
Loads settings from (in priority order):
  ...
  2. Config file (<plugin-data-dir>/config.json)
  3. Defaults
...
_CONFIG_DIR = Path(os.environ.get("CLAUDE_PLUGIN_DATA") or Path.home() / ".elephantbroker")
...
async def _ensure_identity_via_sdk() -> str:
    """Stub: Cognee SDK mode is not available in EB mode."""
    return ""
...
async def _ensure_local_databases() -> None:
    """Stub: Cognee local database setup is not available in EB mode."""
    pass


async def ensure_cognee_ready(config: dict) -> None:
    """Stub: Cognee setup is handled by the EB service, not the plugin."""
    ...
    # Local Cognee SDK mode is not available in EB mode.
    pass


async def ensure_dataset_ready(dataset: str, user) -> None:
    """Stub: Cognee SDK dataset preparation is not available in EB mode."""
    pass
