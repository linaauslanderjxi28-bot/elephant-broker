"""Shared helpers across plugin hook scripts — EB-only edition.

Supports only ElephantBroker backend mode. Cognee HTTP/local SDK fallback
...
# Plugin data directory. Uses CLAUDE_PLUGIN_DATA if available (set by the
# harness for official plugin-hooks users), otherwise falls back to
# ~/.elephantbroker.
...
# EB (ElephantBroker) mode is always active in this file.
_EB_MODULE = None  # lazy-imported on first use
...
def load_resolved(session_key: str = "") -> dict:
    """Load runtime state for the active session (EB mode only)."""
    resolved: dict = {}
    ...
    service_url = _service_url().strip()
    if service_url:
        resolved["service_url"] = service_url

    dataset = str(os.environ.get("COGNEE_PLUGIN_DATASET") or "").strip()
    if not dataset:
        try:
            from config import get_dataset, load_config  # type: ignore
...
def hook_log(event: str, detail: Optional[dict] = None) -> None:
    """Append one structured line to the plugin data dir hook.log.
...
def notify(msg: str) -> None:
    ...
    line to the activity log so saves that happen in async hooks are
    ``tail -f``-visible.
    """
    line = f"claude-plugin: {msg}"
...
def quiet_hook_output(label: str):
    ...
    Claude parses stdout for JSON on hooks such as UserPromptSubmit. Some
    dependencies write directly to file descriptors, so redirect at the OS
    fd level instead of relying only on Python's redirect_stdout.
    """
    _PLUGIN_DIR.mkdir(parents=True, exist_ok=True)
...
def remember_pending_prompt(
    ...
    """Store the current prompt until the Claude Code Stop hook provides the assistant answer."""
...
def pop_pending_prompt(session_id: str, *, turn_id: str = "") -> dict:
    """Return and remove the prompt saved for this Claude Code turn."""
    if not session_id:
...
def _service_url() -> str:
    ...
    return (os.environ.get("COGNEE_SERVICE_URL") or "http://localhost:8420").strip().rstrip("/")


def _eb_module():
    """Lazy-import and return the EB adapter module."""
    global _EB_MODULE
...
def http_api_ready() -> bool:
    """In EB mode, always report ready as long as we have a service URL."""
    return bool(_service_url())


def resolve_runtime_mode() -> dict:
    """Resolve hook runtime mode (always 'eb' in this edition)."""
    url = _service_url()
    return {
        "mode": "eb",
        "service_url": url,
...
def _entry_to_text(entry: dict) -> str:
    """Convert a Cognee-style QA/trace entry dict to plain text for EB store."""
...
def remember_entry_via_http(
    ...
    *,
    ...
    """Stage a typed QA/trace entry for final sync to EB.

    Interactive hooks stay fast by staging locally. The staged data is
    flushed through ``/memory/ingest-turn`` during session-end sync.
    """
