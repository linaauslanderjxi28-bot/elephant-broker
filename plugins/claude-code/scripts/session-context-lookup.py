#!/usr/bin/env python3
"""Search session, trace, and graph-context memory for prompt-relevant context.

Runs on the Claude Code `UserPromptSubmit` hook. Uses the ElephantBroker
backend routing path to retrieve recent session context, tool trace context,
and higher-level graph-context summaries when available.

Configuration:
    Resolves session state through EB mode only.
"""
...
async def _run(prompt: str) -> dict | None:
    ...
    try:
        ...
        _state = _Path(os.environ.get("CLAUDE_PLUGIN_DATA") or _Path.home() / ".elephantbroker") / "last_recall.json"
    ...
    try:
        ...
        _audit = _Path(os.environ.get("CLAUDE_PLUGIN_DATA") or _Path.home() / ".elephantbroker") / "recall-audit.log"
