#!/usr/bin/env bash
# ElephantBroker status line for Codex.
# Reads small JSON state files written by the elephantbroker-memory plugin
# (~/.elephantbroker/) and renders a one-line summary at the bottom
...
static="eb[$mode] ds=${dataset} sess=${sess_short}"
