...
message from the Stop hook as a remembered QA-style turn.
"""
...
def _fire_improve_background(dataset: str, session_id: str, reason: str) -> None:
...
async def _store_tool_call(payload: dict) -> None:
    ...
    # Suppress self-reference: any Bash/run_command call that mentions 'elephantbroker'
    # is likely the plugin/CLI talking to itself and would recurse.
    if tool_name == "run_command":
        cmd = ""
        if isinstance(tool_input, dict):
            cmd = str(tool_input.get("CommandLine", ""))
        if "elephantbroker" in cmd:
            hook_log("skip_self_eb_run_command", {"cmd_prefix": cmd[:80]})
            return
    ...
    session_id, dataset, _ = _load_session()
    if not session_id:
    ...
    if result:
        ...
        if should_improve:
            _fire_improve_background(dataset, session_id, reason=f"turn_{count}")
    else:
